...
class Player(pygame.sprite.Sprite):
    def __init__(self, game_mode):
        ...
        if self.game_mode == "Neuroevolution":
            ...
            layer_sizes = [6, 4, 1]  # TODO (Design your architecture here by changing the values)
            self.nn = NeuralNetwork(layer_sizes)

    def euclidean_distance(self, player_x, player_y, obstacle_x, obstacle_y):
        return math.sqrt((player_x - obstacle_x) ** 2 + (player_y - obstacle_y) ** 2)

    def normalize(self, x):
        normalized_x = x
        if np.sqrt(np.sum(x**2)) > 0:
            normalized_x = x / np.sqrt(np.sum(x**2))
        return normalized_x

    def init_input_vector(self, obstacles, player_x, player_y):
        input_vector = np.zeros((6, 1))
        # print('len(obstacles):', len(obstacles))
        for i in range(min(len(obstacles), 2)):
            cur_obstacle = obstacles[i]
            input_vector[3*i] = cur_obstacle['x']
            input_vector[3*i+1] = cur_obstacle['y']
            input_vector[3*i+2] = self.euclidean_distance(player_x, player_y, cur_obstacle['x'], cur_obstacle['y'])
        normalized_input_vector = self.normalize(input_vector)
        return normalized_input_vector
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        input_vector = self.init_input_vector(obstacles, player_x, player_y)
        output_vector = self.nn.forward(input_vector, activation_type='sigmoid')
